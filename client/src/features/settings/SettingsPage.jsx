...
import {
  ...
  Camera, Crown, IndianRupee, Zap, CheckCircle2,
} from "lucide-react";
...
// ── Plan config — mirrors plans.config.js ─────────────────────────────────────
const PLAN_FEATURES = {
  free:       ["Unlimited during trial", "All features unlocked", "WhatsApp billing", "UPI payment links", "Analytics"],
  pro:        ["Unlimited lanes & customers", "WhatsApp blast", "UPI payment links", "Analytics", "Priority support"],
  enterprise: ["Everything in Pro", "Dedicated support", "Custom integrations", "SLA guarantee"],
};

const PLAN_LABELS  = { free: "Free Trial", pro: "Pro", enterprise: "Enterprise" };
const PLAN_PRICES  = { free: null, pro: 699, enterprise: 1299 };
const PLAN_COLORS  = {
  free:       { badge: "bg-gray-100 text-gray-600",   card: "border-gray-200" },
  pro:        { badge: "bg-amber-100 text-amber-700",  card: "border-amber-200 bg-amber-50/30" },
  enterprise: { badge: "bg-violet-100 text-violet-700", card: "border-violet-200 bg-violet-50/30" },
};

const WHATSAPP_NUMBER = "919834439861";

function SubscriptionCard({ tenant }) {
  ...
  const msLeft     = isFree ? new Date(tenant.trialEndsAt) - new Date() : null;
  const daysLeft   = msLeft !== null ? Math.ceil(msLeft / 86_400_000) : null;
  const trialEnd   = isFree && tenant.trialEndsAt
    ? new Date(tenant.trialEndsAt).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })
    : null;
  const expired    = daysLeft !== null && daysLeft <= 0;

  const waMsg      = encodeURIComponent(`Hi, I'd like to upgrade my DairyFlow plan to Pro. Current account: ${tenant.phone || ""}`);
  const waLink     = `https://wa.me/${WHATSAPP_NUMBER}?text=${waMsg}`;

  return (
    <div className={`bg-white border rounded-2xl p-6 space-y-5 ${colors.card}`}>
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-sm font-bold text-gray-900">Subscription</h2>
          <p className="text-xs text-gray-500 mt-0.5">Your current plan and billing</p>
        </div>
        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold flex-shrink-0 ${colors.badge}`}>
          <Crown size={11} />
          {label}
        </span>
      </div>
      ...
      {isFree && (
        <div className={`rounded-xl px-4 py-3 text-sm font-medium
          ...
          {expired
            ? "⚠️ Your trial has ended. Upgrade to continue using all features."
            : `✨ Trial active · ${daysLeft} day${daysLeft !== 1 ? "s" : ""} remaining · Ends ${trialEnd}`}
        </div>
      )}

      {/* Paid plan — show renewal info */}
      {!isFree && price && (
        <div className="bg-gray-50 border border-gray-100 rounded-xl px-4 py-3">
          <p className="text-sm font-bold text-gray-900">₹{price}<span className="text-xs font-normal text-gray-500">/month</span></p>
          <p className="text-xs text-gray-500 mt-0.5">Contact us to manage your subscription</p>
        </div>
      )}
      ...
      {/* Upgrade CTA — only for free or if they want to change */}
      {isFree && (
        <div className="pt-1 border-t border-gray-100">
          <p className="text-xs text-gray-500 mb-3">
            Upgrade to keep full access after your trial. Plans start at ₹699/month.
          </p>
          <div className="flex flex-col sm:flex-row gap-2">
            <a href={waLink} target="_blank" rel="noopener noreferrer"
              className="flex-1 flex items-center justify-center gap-2 bg-gray-900 hover:bg-black
                text-white text-sm font-bold py-2.5 rounded-xl transition">
              <Zap size={14} /> Upgrade Plan
            </a>
            <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noopener noreferrer"
              className="flex-1 flex items-center justify-center gap-2 border border-gray-200
                text-gray-700 text-sm font-semibold py-2.5 rounded-xl hover:bg-gray-50 transition">
              Chat with us
            </a>
          </div>
        </div>
...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState({
    ...
    address: "", invoicePrefix: "INV", upiId: "",
  });
  ...
  const validate = () => {
    ...
    if (form.upiId && !/^[a-zA-Z0-9.\-_+]+@[a-zA-Z0-9]+$/.test(form.upiId.trim()))
      e.upiId = "Enter a valid UPI ID (e.g. 9876543210@ybl)";
    setErrors(e);
  ...
  const handleFileSelect = (e) => {
    ...
    if (!["image/jpeg","image/jpg","image/png","image/webp"].includes(file.type)) {
      setUploadErr("Only JPEG, PNG and WebP files are allowed."); return;
    }
    if (file.size > 2 * 1024 * 1024) { setUploadErr("Logo must be smaller than 2MB."); return; }
    setUploadErr(""); setLogoFile(file);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Manage your dairy's profile and subscription</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Shown on PDF bills. JPEG, PNG or WebP · max 2MB.</p>
        </div>
          ...
          <div className="space-y-3 flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => fileInputRef.current?.click()}
                className="inline-flex items-center gap-1.5 text-xs font-semibold border border-gray-200 px-3 py-2 rounded-xl hover:bg-gray-50 transition text-gray-700">
                <Upload size={12} /> Choose file
              </button>
              {logoFile && (
                <button onClick={handleUploadLogo} disabled={uploading}
                  className="inline-flex items-center gap-1.5 text-xs font-semibold bg-gray-900 text-white px-3 py-2 rounded-xl hover:bg-black disabled:opacity-50 transition">
                  {uploading ? <><Loader2 size={12} className="animate-spin" /> {uploadProgress}%</> : <><Upload size={12} /> Upload</>}
                </button>
              )}
              {tenant?.logoUrl && !logoFile && (
                <button onClick={handleRemoveLogo} disabled={removingLogo}
                  className="inline-flex items-center gap-1.5 text-xs font-semibold border border-red-200 text-red-500 px-3 py-2 rounded-xl hover:bg-red-50 disabled:opacity-50 transition">
                  {removingLogo ? <Loader2 size={12} className="animate-spin" /> : <Trash2 size={12} />} Remove
                </button>
              )}
            </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">These details appear on every PDF bill sent to customers.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Invoice Prefix" icon={FileText}  value={form.invoicePrefix} onChange={handleChange("invoicePrefix")} placeholder="INV" maxLength={6}   error={errors.invoicePrefix}
            hint="e.g. INV → bill numbers like INV-001" />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <input type="text" value={form.upiId} onChange={handleChange("upiId")}
          ...
          {errors.upiId
            ...
            : <p className="text-xs text-gray-400">When set, a tap-to-pay UPI link is added to every WhatsApp bill automatically.</p>}
          ...
          {form.upiId && !errors.upiId && (
            <div className="flex items-center gap-2.5 bg-green-50 border border-green-100 rounded-xl px-3.5 py-2.5 mt-1">
              ...
              <div>
                <p className="text-xs font-semibold text-green-800">Pay link active · {form.upiId.trim()}</p>
                <p className="text-[10px] text-green-600 mt-0.5">Tap-to-pay link included in every WhatsApp message</p>
              </div>
            </div>
          )}
        </div>
        ...
        {saveErr && (
          <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 border border-red-100 px-4 py-3 rounded-xl">
            <AlertCircle size={14} className="flex-shrink-0" />{saveErr}
          </div>
        )}
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">How your business info appears on customer bills.</p>
        </div>
