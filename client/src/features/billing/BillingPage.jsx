...
// ── Fetch latest bill for a single customer ───────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
function WhatsAppBlastModal({ customers, outstandingMap, laneName, dairyName, onClose }) {
  const [billMap, setBillMap]   = useState({});
  const [loading, setLoading]   = useState(true);
  const [selected, setSelected] = useState(new Set());
  const [search, setSearch]     = useState("");

  // Send states — queue-based (popup-blocker safe)
  const [sendQueue, setSendQueue]   = useState(null);  // null | { items, index, results }
  const [sendDone, setSendDone]     = useState(false);
  const [sendResults, setSendResults] = useState([]);

  // ── Load latest bills for every customer on mount ─────────────────────────
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setLoading(true);
      ...
      if (!cancelled) {
        ...
        setBillMap(map);
        // Auto-select: has bill + has phone
  ...
  // ── Build queue on "Send" click ───────────────────────────────────────────
  const handleSend = () => {
    const toSend = customers.filter((c) => selected.has(c._id));
    if (!toSend.length) return;

    // Pre-classify: skip ineligible immediately, queue the rest
  ...
  // ── User clicks "Open WhatsApp" for current queue item ────────────────────
  const handleQueueOpen = () => {
    if (!sendQueue) return;
    const { items, index, results } = sendQueue;
    const { customer, bill } = items[index];

    // THIS is the direct user click → window.open always works here
    openWhatsApp({ phone: customer.phone, message: buildWhatsAppMessage({ bill, customer, dairyName }) });
    ...
    if (nextIndex >= items.length) {
      ...
      setSendQueue({ items, index: nextIndex, results: newResults });
    }
  };

  // ── User skips current queue item ─────────────────────────────────────────
  ...
  // ── View: queue step-through (popup-blocker safe) ────────────────────────
  if (sendQueue) {
    const { items, index, results } = sendQueue;
    const current   = items[index];
    const sentSoFar = results.filter(r => r.status === "sent").length;
    const total     = items.length + results.filter(r => r.status === "skipped" && r.reason !== "Skipped by admin").length;
    // progress = already processed (sent + auto-skipped from results) + current position
    const done      = results.length;
    const grandTotal = items.length + results.filter(r => r.reason !== "Skipped by admin").length;
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                  <Phone size={10} /> {current.customer.phone}
                </p>
              </div>
              {(() => {
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 border border-gray-200 text-xs font-semibold py-2.5 rounded-xl hover:bg-gray-50 transition text-gray-500"
            >
              Skip
            </button>
            <button
              onClick={handleQueueOpen}
              className="flex-[2] flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20b858] text-white text-sm font-bold py-2.5 rounded-xl transition"
            >
              <WAIcon size={15} />
              Open WhatsApp
            </button>
          ...
          <p className="text-[11px] text-gray-400 text-center">
            Click "Open WhatsApp" → send the message → come back and continue
          </p>
        </div>
      </ModalShell>
    );
      ...
      <ModalShell onClose={onClose} maxWidth="max-w-md">
        <div className="p-6 space-y-5">
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={() => { setSendDone(false); setSendResults([]); }} className="flex-1 border border-gray-200 text-sm font-semibold py-2.5 rounded-xl hover:bg-gray-50 transition text-gray-700">
              Send Again
            </button>
            <button onClick={onClose} className="flex-1 bg-gray-900 hover:bg-black text-white text-sm font-semibold py-2.5 rounded-xl transition">
              Done
            </button>
          </div>
        </div>
  ...
  return (
    <ModalShell onClose={onClose} maxWidth="max-w-2xl">
      ...
      <div className="flex items-start justify-between px-6 py-5 border-b border-gray-100">
        ...
        <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 hover:bg-gray-100 transition text-gray-500">
          <X size={15} />
        </button>
      </div>
        ...
        {loading ? (
          ...
          filtered.map((customer) => {
            ...
            return (
              <div
                key={customer._id}
                onClick={() => isEligible && toggleOne(customer._id)}
              ...
              >
                ...
                <div className="flex-shrink-0">
                  {isEligible
                    ? isSelected ? <CheckSquare size={17} className="text-green-600" /> : <Square size={17} className="text-gray-300" />
                    : <Square size={17} className="text-gray-200" />
                  }
                </div>

                {/* Left: name + phone + bill period */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-sm font-semibold text-gray-900 truncate">{customer.name}</span>
                    {!customer.phone && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-red-50 text-red-500 border border-red-100 flex-shrink-0">
                        <PhoneOff size={8} /> No phone
                      </span>
                    )}
                    {customer.phone && !bill && (
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-400 flex-shrink-0">No bill</span>
                    )}
                  </div>
                  ...
                  {bill ? (
                    <>
                      ...
                      {bill.status !== "PAID" && pending > 0 && (
                        <p className="text-[11px] text-red-500 font-semibold">₹{pending} due</p>
                      )}
                    </>
                  ) : (
      ...
      {!loading && (
        <div className="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-gray-400">
            {selected.size > 0
              ? `WhatsApp will open ${selected.size} tab${selected.size !== 1 ? "s" : ""} · allow popups`
              : "Select customers to send"
            }
          </p>
          <div className="flex gap-2">
            <button onClick={onClose} className="px-4 py-2.5 text-xs font-semibold text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50 transition">
              Cancel
            </button>
...
function ModalShell({ children, onClose, maxWidth = "max-w-sm" }) {
  return (
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-start justify-center overflow-y-auto px-4 py-6">
      <div className={`bg-white rounded-2xl shadow-2xl w-full ${maxWidth} mb-8`}>
        {children}
      </div>
    </div>
  );
...
const BillingPage = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 space-y-5">
      ...
      {/* Lane selector + WhatsApp Blast button */}
