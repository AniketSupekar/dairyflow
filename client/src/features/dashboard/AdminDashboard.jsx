...
const AdminDashboard = () => {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      setStats({ totalCustomers: d.totalCustomers, totalLanes: d.totalLanes, deliveriesToday: d.deliveriesToday, pendingBills: d.pendingBills });
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
          ...
          {[
            ...
            return (
              <Link
                key={a.label}
                to={a.to}
                className="group flex items-center gap-3.5 bg-white border border-gray-100 rounded-2xl px-4 py-3.5 hover:bg-gray-50/80 transition"
              >
      ...
      <div className="flex items-center gap-3 bg-white border border-gray-100 rounded-2xl px-4 py-3.5">
        ...
        <Link to="/admin/deliveries" className="text-xs font-semibold text-gray-400 hover:text-gray-700 transition flex items-center gap-1 flex-shrink-0">
    ...
    <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
      ...
      <div className="divide-y divide-gray-50">
        {lanes.slice(0, 6).map((lane) => {
          ...
          const pct = lane.totalCustomers > 0 ? Math.round((recorded / lane.totalCustomers) * 100) : 0;
          ...
          return (
            ...
            >
              {/* Status dot */}
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${lane.isComplete
                  ? "bg-emerald-400"
                  : recorded > 0 ? "bg-amber-400" : "bg-gray-200"}`}              />
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${lane.isComplete ? "bg-emerald-400" : "bg-amber-400"}`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
      ...
      <Link
        to="/admin/delivery-summary"
        className="flex items-center justify-center gap-1.5 px-4 py-3 border-t border-gray-50 text-xs font-semibold text-gray-400 hover:text-gray-700 hover:bg-gray-50/60 transition"
      >
...
function StatCard({ label, value, loading, icon: Icon, accent, to, alert }) {
  ...
  const inner = (
    <div className={`relative bg-white border rounded-2xl p-4 hover:shadow-sm transition-all group cursor-pointer overflow-hidden ${alert ? "border-rose-200 hover:border-rose-300" : "border-gray-100 hover:border-gray-200"}`}    >
...
function SectionLabel({ children }) {
  return <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest px-0.5">{children}</p>;
}
